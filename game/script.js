...
class Player {
    ...
    update(){
        ...
        let theta = Math.atan2(dy, dx);  // make fish head to mouse click
        this.angle = theta;
        if (mouse.x != this.x){
            this.x -= dx/20;
        }
...
const bubblePop1 = document.createElement('audio');  // getting sound 
...
function handleBubble(){
    ...
    for  ( let i = 0; i < bubblesArray.length; i++ ){
        if ( bubblesArray[i].y < 0 - bubblesArray[i].radius * 2){
            bubblesArray.splice(i, 1)    // cut element at some point 
        }
        if (bubblesArray[i]){
            if ( bubblesArray[i].distance < bubblesArray[i].radius + player.radius ){
                if (!bubblesArray[i].counted){
                    if (bubblesArray[i].sound == 'sound1'){
                        bubblePop1.play();  // playing sound
                    } else {
                        bubblePop2.play();
                    }
                    score++;
